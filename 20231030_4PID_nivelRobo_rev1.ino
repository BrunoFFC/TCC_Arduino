...
//fi
// angulo 0 - sem curvas
// Variar de -90º(270º) a 90º
// angulo PI/4 (45º) - curva para esquerda
// angulo PI/2 (90º) - curva para esquerda
// angulo 7*PI/4 (315º ou -45º) - curva para a direita
// angulo 3*PI/2 (270º ou -90º) - curva para a direita
double ang_graus = 0; // angulo em graus
...
double output_robo_dir = 0; //velocidade a ser controlada em pwm para o lado direito(motores 2 e 3)
double output_robo_esq = 0; //velocidade a ser controlada em pwm para o lado esquerdo(motores 1 e 4)
...
double veloc_ref_robo_ms = 1; // Velocidade desejada para o robô(m/s)


// Cria PID para controle
//PID myPID(&Input, &Output, &Veloc_ref, Kp, Ki, Kd, DIRECT or INVERSE);
//Motores 1 e 4
PID motorPID_1(&Veloc_inercial_x, &output_robo_esq, &veloc_ref_robo_ms, KP, KI, KD, DIRECT);//(input,output,Veloc_ref,direct)
//Motor 2 e 3
PID motorPID_2(&Veloc_inercial_x, &output_robo_dir, &veloc_ref_robo_ms, KP, KI, KD, DIRECT);
// Todos os motores
...
//Entradas dos valores de pwm que desejo para cada roda e a direção da roda. 
  ...
  dir1 = 0; // Set direction (1 sentido anti-horário e 0 sentido horário)
  ...
  Serial.println("PWM_robo\tVeloc_esquerda[rad/s]\tVeloc_direita[rad/s]\tveloc_local[m/s]\tVeloc_x[m/s]\tVeloc_y[m/s]\tVeloc_ref_robo_ms:");
