...
float erro_1_vw, erro_2_vw;



double veloc_linear_ref;
double veloc_ang_ref; 
...
void calculaVelocidadeReferencia()
{
  ...
  veloc_ang_robo = (theta_ponto_1 - theta_ponto_2)*raio_roda/2*b; //(rad/s)
}
...
//Entradas dos valores de pwm que desejo para cada roda e a direção da roda. 
  ...
  {// Calcula a cada 1 segundo, a velocidade.
    ...
    veloc_linear_out = (theta_ponto_1_out + theta_ponto_2_out)*raio_roda/2;//m/s
    veloc_ang_robo_out = (theta_ponto_1_out - theta_ponto_2_out)*raio_roda/2*b;//rad/s
    // Cálculo do erro da velocidade na cinematica inversa
    erro_1_vw = veloc_linear_ref - veloc_linear_out;
    erro_2_vw = veloc_ang_ref - veloc_ang_robo_out;

  }

  //Velocidade de referencia
  veloc_linear_ref = 0.2;
  veloc_ang_ref = 0.02;// valor positido movimento para o lado direito. Valor negativo movimento para o lado esquerdo.
...
/*
  // Parar as rodas depois de um tempo em movimento
  if (tempoatual >= duracaoOperacao)
  {
    pwm1 = 0;
    pwm2 = 0;
    pwm3 = 0;
